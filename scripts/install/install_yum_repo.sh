...
main ()
{
  ...
  yum_repo_config_url="https://${master_token}:@packagecloud.io/install/repositories/shapeblue/ccs/config_file.repo?os=${os}&dist=${dist}&name=${unique_id}&source=script"
  echo "Found unique id: ${unique_id}"

  yum_repo_path=/etc/yum.repos.d/shapeblue_ccs.repo
  ...
  yum install -y pygpgme --disablerepo='shapeblue_ccs'
  ...
  if [ "$?" != "0" ]; then
    ...
    sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/shapeblue_ccs.repo
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='shapeblue_ccs'
  ...
  echo "Generating yum cache for shapeblue_ccs..."
  yum -q makecache -y --disablerepo='*' --enablerepo='shapeblue_ccs'

  echo
  echo "The repository is setup! You can now install the CloudStack Container Service."
}
